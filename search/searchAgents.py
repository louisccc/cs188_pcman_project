...
def cornersHeuristic(state, problem):
    ...
    node = state[0]
    while len(unvisited):
        est_dist, corner = min([(util.manhattanDistance(node, corner), corner) for corner in unvisited])
        min_dist += est_dist
        node = corner
...
def foodHeuristic(state, problem):
    ...
    node = position
    while len(food_positions):
        est_dist, position = min([(util.manhattanDistance(node, position), position) for position in food_positions])
        min_dist += est_dist
        node = position
        food_positions.remove(node)
